...
- [x] Add explicit converted body rest transforms and parent/child joint anchors to
      both versioned physics rigs; refresh catalog hashes and provenance.
- [x] Tighten schema and `PhysicsRigDescriptor` validation for unique names, exact
      topology, unit axes, finite ordered limits, frame ownership, anchors,
      collision policy, and model/rig identity.
- [x] Add descriptor-negative and rig-to-manifest rest/axis parity tests for SY205
      and SY135 before constructing any runtime body.

## Phase B: Complete Jolt Rig Ownership

- [x] Refactor `JoltChassisTrackRuntime` from `_body` to a five-body registry while
      preserving the Phase 1 chassis/track API and single-owner teardown.
- [x] Create slew plus boom/arm/bucket joints, collision filtering, mass/inertia,
      COM, damping, limits, and finite-state guards from the selected descriptor.
- [x] Subscribe the controller to authority changes and rebuild the entire rig on
...
- [x] Route the four accepted operator axes and command identity into the runtime.
- [x] Implement target shaping, acceleration/jerk limits, damping, effort/load
      saturation, holding, limit anticipation, and invalid-input disarm.
- [x] Add one tick-boundary bucket payload mass/local-COM adapter with identity,
      clamps, and rebuild/reset clearing.

## Phase D: Shared Snapshot, Visuals, And Truth

- [x] Capture one immutable post-step snapshot for five bodies, four joints,
      contacts, payload, terrain, model/rig identity, epoch, tick, and quality.
- [x] Make `MotionPresentation` consume that snapshot in Jolt mode; preserve the
      Python path only in `python_kinematic`.
- [x] Drive the SY205 passive four-bar after physical arm/bucket pose without adding
      physical bodies or a closed loop.
- [x] Make `SimulationTruthPublisher` serialize the runtime snapshot directly;
      remove Phase 1 frozen body/joint placeholders and quality flags.
- [x] Disable the legacy bucket-ground lift writer in Jolt-authoritative mode while
      leaving terrain mutation for Phase 3.

## Phase E: Verification And Tuning

- [x] Test each joint in both directions and at limits for both models, including
      rest-frame parity and target-versus-actual-versus-effort telemetry.
- [x] Test mixed-axis motion, load slowdown/holding, chassis reaction, SY205 passive
      linkage invariants, and long-run energy/finite bounds.
- [x] Test reset, disconnect, authority epoch, failed descriptor, model switch,
      profile rollback, neutral re-arm, and zero residual bodies/joints.
- [x] Run `pixi run verify`, `pixi run backend-smoke`, the full Godot standalone
      matrix, and Godot AI MCP live operation for SY205 and SY135; record fixed-step,
      solver, and rebuild metrics.
- [x] Update model, visual, client-boundary, runtime-profile, truth, and test docs
