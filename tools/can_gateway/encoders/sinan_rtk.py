...
Byte layouts mirror ProtocolParser::parseCgi610*. Per product decision the
whole family is LITTLE-endian (including the velocity frame, overriding the
parser code's big-endian structure):
...
Encoders default to the parser-parity convention above; decoders accept an
explicit byteorder for cross-checks.
"""
...
def encode_time_frame(week: int, gps_seconds: float) -> bytes:
    ms = int(round(gps_seconds * 1000.0)) & 0xFFFFFFFF
    return struct.pack("<HI2x", week & 0xFFFF, ms)


def encode_status_frame(gps_age_cs: int = 12) -> bytes:
    return struct.pack(
        "<BBBBHBB",
        _STATUS_SYSTEM_FIX,
        _STATUS_SATS_USED,
...
def encode_lon_frame(lon_deg: float, byteorder: str = "little") -> bytes:
    return struct.pack(f"{_endian(byteorder)}q", int(round(lon_deg * 1e8)))


def encode_lat_frame(lat_deg: float, byteorder: str = "little") -> bytes:
    return struct.pack(f"{_endian(byteorder)}q", int(round(lat_deg * 1e8)))


def encode_alt_frame(alt_m: float, byteorder: str = "little") -> bytes:
    return struct.pack(f"{_endian(byteorder)}i4x", int(round(alt_m * 1000.0)))


def encode_velocity_frame(ve: float, vn: float, vu: float, speed: float, byteorder: str = "little") -> bytes:
    def centi(value: float) -> int:
        return max(-32768, min(32767, int(round(value * 100.0))))

    return struct.pack(
        f"{_endian(byteorder)}hhhh", centi(ve), centi(vn), centi(vu), centi(speed)
    )


def encode_heading_frame(heading_deg: float, byteorder: str = "little") -> bytes:
    counts = int(round((heading_deg % 360.0) * 100.0)) & 0xFFFF
...
def build_rtk_frames(state, byteorder: str = "little") -> dict[int, bytes]:
    ...
    unix_s = state.wall_clock_unix_s() if hasattr(state, "wall_clock_unix_s") else _default_wall_clock()
    ...
    return {
        ...
        RTK_IDS_ORDERED[1]: encode_status_frame(),
        ...
        RTK_IDS_ORDERED[8]: encode_velocity_frame(ve, vn, vu, speed, byteorder),
